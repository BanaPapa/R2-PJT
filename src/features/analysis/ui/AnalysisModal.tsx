...
import { toExportMarkdown, toExportJson, toExportCsv, downloadTextFile, exportBaseName } from '../lib/export';
...
export const AnalysisModal: React.FC<AnalysisModalProps> = ({ open, onClose }) => {
  ...
  // 직접 선택 예상치: 정적 JSON 비동기 로드 → 디바운스로 갱신.
  useEffect(() => {
    if (phase !== 'idle' || panel !== 'custom') return;
    if (pickedRegions.length === 0 || selTabs.size === 0 || pickedRegions.length > MAX_ANALYSIS_REGIONS) {
  ...
  // 분석 대상 지역은 최대 3개. 초과 시 알림을 띄우고 진행하지 않는다(그래프 비교는 5개까지 별개).
  ...
  const doExport = (fmt: 'md' | 'json' | 'csv') => {
    setExportOpen(false);
    ...
    else downloadTextFile(`${base}.csv`, toExportCsv(resultScope, ds), 'text/csv');
  };
        ...
        <div className="min-h-0 flex-1 overflow-auto px-5 py-4">
          {phase === 'idle' && (
            showManager ? (
              ...
              <>
                ...
                <div className="mb-4 flex items-center justify-between">
                  <div className="inline-flex rounded-lg bg-gray-100 p-0.5">
                    {(['current', 'custom', 'slot'] as Panel[]).map(p => (
                      <button
                        key={p}
                        onClick={() => { setPanel(p); setRegionNotice(null); }}
                ...
                {regionNotice && (
                  <div className="mb-3 flex items-start gap-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-700">
                    <svg className="mt-0.5 h-4 w-4 flex-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" /></svg>
          ...
          {phase === 'error' && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <p className="mb-4 max-w-md text-sm text-red-600">{friendlyError(error)}</p>
        ...
        {phase === 'done' && (
          <div className="flex flex-none items-center gap-2 border-t border-gray-200 px-5 py-3">
            ...
            <div className="relative flex-none">
              ...
              {exportOpen && (
                <div className="absolute bottom-full right-0 mb-1 w-44 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
                  <button onClick={() => doExport('md')} className="block w-full px-3 py-2 text-left text-sm hover:bg-gray-50">Markdown (.md) · 추천</button>
                  <button onClick={() => doExport('json')} className="block w-full px-3 py-2 text-left text-sm hover:bg-gray-50">JSON (.json)</button>
                  <button onClick={() => doExport('csv')} className="block w-full px-3 py-2 text-left text-sm hover:bg-gray-50">CSV (.csv)</button>
                </div>
              )}
            </div>
            <button
